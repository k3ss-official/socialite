...
> Version 0.1 · Owner: Tony Whelan (k3ss) · Status: DRAFT — design doc; reconcile with implemented socialite/stages/bible.py + schemas/ (implementation predates this spec and already covers much of it; the delta is D5 compliance exposure, the upsell-map fields, and evidence rules)
...
What usable photography exists (their own posts, tagged photos with rights caution flagged) vs. what BUILD will need generated. Output feeds STYLE-LOCK for the Higgsfield/CD-harness pass. Never inventory third-party photos as usable without a rights note.
...
Rung types (residual ladder alignment — see config/ladder.yaml):
...
## 6. BIBLE.JSON SCHEMA (pinned — breaking changes bump the version; reconcile with schemas/*.schema.json)
